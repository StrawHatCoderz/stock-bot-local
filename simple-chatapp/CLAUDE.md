...
- **`src/models/agent-session.ts`** (`AgentSession`) — wraps the Claude Agent SDK's `query()`. Takes user input through `models/message-queue.ts`'s `MessageQueue` (push-based async iterator, since the SDK expects an async-iterable prompt for multi-turn streaming) and exposes an output stream the `Session` consumes. Imports `MCP_HOST`/`getAllowedToolsForRole`/`buildSystemPrompt` from `ai-client.ts`.
- **`src/ai-client.ts`** — no longer a class file. Holds the `AgentSession`-adjacent config helpers: `MCP_HOST`, `getAllowedToolsForRole`, and the `UserMessage` type consumed by `models/message-queue.ts` — plus a re-export of `buildSystemPrompt`, whose implementation now lives in `src/prompts/` (see "System prompt structure" below).
...
`ai-client.ts` defines every MCP tool as a `McpTool` string-enum member (`mcp__<server>__<tool>`), grouped into a `TOOL_GROUPS` object — `read` (6: `search_areas_fuzzy`, `search_products_fuzzy`, `validate_area`, `validate_product`, `list_areas` on validation-mcp, plus `get_stock` on stock-mcp), `adjustments` (`create_adjustment`, `get_adjustment_threshold`, both stock-mcp), `zeroization` (`create_zeroization`, `create_area_zeroization`, both stock-mcp), `transfers` (5, all transfer-mcp: `create_transfer`, `list_outgoing_transfers`, `list_incoming_transfers`, `approve_transfer`, `list_stores`), and `admin` (3, all admin-mcp: `list_store_managers`, `list_store_associates`, `set_associate_threshold`). `get_adjustment_threshold` proxies a read-only `GET /api/stock/adjustment-threshold` endpoint on `stock-service`, returning a `STORE_ASSOCIATE` caller's `thresholdPercent`/`usedPercent`/`remainingPercent` for a specific product (`applicable: false` for `STORE_MANAGER`, who has no ceiling). `list_areas` takes no params and returns every area in the caller's own store (storeId is derived server-side from the verified session token, same as the other validation tools — see "Identity flow" above) — it's for "what areas are in my store?"-style questions, not the fuzzy-search-then-validate workflow. Unlike `list_areas`, the transfer tools that operate on a specific store (`create_transfer`'s `fromStoreId`/`toStoreId`, both listing tools' `storeId`, `approve_transfer`'s implicit destination) take an explicit store identifier as a real argument rather than having it derived server-side — see "Identity flow" above for why `buildSystemPrompt` states the caller's actual `storeId` value; `list_stores` is the one transfer tool that needs no store argument at all, since it always means "every store except my own." `authenticate_user`/`get_user_details` exist on the MCP server but are deliberately excluded — the agent never authenticates itself.

A `ROLE_TOOLS` object composes these groups per role: `STORE_MANAGER` gets `read` + `adjustments` + `zeroization` + `transfers` (15 tools total); `STORE_ASSOCIATE` gets `read` + `adjustments` only (omitting zeroization and every transfer tool, which stay Manager-only); `ADMIN` gets only the `admin` group (entirely disjoint from every other role — an Admin session never has access to any read-only, zeroisation, adjustment, or transfer tool, and no other role has access to `admin`'s three).

`agent-session.ts` doesn't build a tool list itself — it calls `getAllowedToolsForRole(identity?.role)` (in `ai-client.ts`), which looks `ROLE_TOOLS` up by role (falling back to just `read` for any other role, or no identity at all). This is a hard, code-level gate so the SDK cannot invoke an excluded tool regardless of what the model decides. This is in addition to, not a replacement for, `StockController.java`'s (and, for transfer, `TransferController.java`'s) own `FORBIDDEN_ROLE` check (see root `CLAUDE.md`'s "RBAC is enforced authoritatively in `stock-service`"). `create_adjustment` is available to both Manager and Associate roles by design — its own role-conditioned quantity floor (an Associate's request can't reduce a product to exactly 0) is enforced by `StockController.java`, not by tool presence, since it depends on the item's live quantity rather than a static role→tool mapping. So is an Associate's per-product adjustment threshold (a depleting quota an Admin sets via `set_associate_threshold`) — see root `CLAUDE.md`'s "Admin is a third role" for the full mechanism — but the chat agent now checks it pre-emptively via `get_adjustment_threshold` before ever calling `create_adjustment`, rather than only finding out reactively from an `ADJUSTMENT_EXCEEDS_THRESHOLD` failure.
