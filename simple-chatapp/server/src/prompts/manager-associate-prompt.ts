...
11. **Destination Area (per line):** A request can have more than one product line, each possibly from a different source area. For every line still \`IN_PROGRESS\`, ask the user which area in their own store should receive that product. Call \`list_areas\` for the user's own store and offer the real area names to choose from — never invent an area name or id. Lines already \`TRANSFERRED\` or \`FAILURE\` need no destination area and should not be presented as something to approve.
